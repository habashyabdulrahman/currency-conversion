...
let goldCache: GoldCacheEntry | undefined;

type Theme = "light" | "dark";
type AppView = "currency" | "gold";
...
class CurrencyConverter {
  private readonly amount = requireElement<HTMLInputElement>("#amount");
  private readonly fromSelect =
    requireElement<HTMLSelectElement>("#fromCurrency");
  private readonly toSelect = requireElement<HTMLSelectElement>("#toCurrency");
  private readonly convertButton =
    requireElement<HTMLButtonElement>("#convertBtn");
  private readonly swapButton = requireElement<HTMLButtonElement>("#swapBtn");
  private readonly resultBox = requireElement<HTMLElement>("#result");
  private readonly errorBox = requireElement<HTMLElement>("#error");
  private readonly loadingBox = requireElement<HTMLElement>("#loading");
  private readonly themeToggle =
    requireElement<HTMLButtonElement>("#themeToggle");
  private readonly presetChips = requireElement<HTMLElement>("#presetChips");
  private readonly copiedToast = requireElement<HTMLElement>("#copiedToast");
  private readonly resultAmount =
    requireElement<HTMLElement>("#result .result-amount");
  private readonly exchangeRate =
    requireElement<HTMLElement>("#result .exchange-rate");
  private readonly lastUpdated =
    requireElement<HTMLElement>("#result .last-updated");
  private readonly themeColorMeta =
    requireElement<HTMLMetaElement>('meta[name="theme-color"]');

  private readonly currencyTab =
    requireElement<HTMLButtonElement>("#currencyTab");
  private readonly goldTab = requireElement<HTMLButtonElement>("#goldTab");
  private readonly currencyPanel = requireElement<HTMLElement>("#currencyPanel");
  private readonly goldPanel = requireElement<HTMLElement>("#goldPanel");

  private readonly goldCurrencySelect =
    requireElement<HTMLSelectElement>("#goldCurrency");
  private readonly refreshGoldButton =
    requireElement<HTMLButtonElement>("#refreshGoldBtn");
  private readonly goldResultBox = requireElement<HTMLElement>("#goldResult");
  private readonly goldLoadingBox = requireElement<HTMLElement>("#goldLoading");
  private readonly goldErrorBox = requireElement<HTMLElement>("#goldError");
  private readonly gold24Price = requireElement<HTMLElement>("#gold24Price");
  private readonly goldKaratSelect =
    requireElement<HTMLSelectElement>("#goldKarat");
  private readonly goldWeight =
    requireElement<HTMLInputElement>("#goldWeight");
  private readonly goldCalculatedValue =
    requireElement<HTMLElement>("#goldCalculatedValue");
  private readonly goldCalculationLabel =
    requireElement<HTMLElement>("#goldCalculationLabel");
  private readonly goldSelectedUnitPrice =
    requireElement<HTMLElement>("#goldSelectedUnitPrice");
  private readonly goldCalculatorError =
    requireElement<HTMLElement>("#goldCalculatorError");
  private readonly goldUpdated = requireElement<HTMLElement>("#goldUpdated");

  private conversionRequestId = 0;
  private goldRequestId = 0;
  private toastTimer: ReturnType<typeof setTimeout> | undefined;
  private errorTimer: ReturnType<typeof setTimeout> | undefined;
  private currenciesReady = false;
  private activeView: AppView = "currency";
  private currentGoldResult: GoldPriceResult | undefined;

  constructor(private readonly config: CurrencyConverterConfig) {
    this.initTheme();
    this.initView();
    this.bindEvents();
  ...
  private setView(view: AppView, loadData = true): void {
  ...
  private async populateCurrencies(): Promise<void> {
  ...
  private populateSelectOptions(
    codes: string[],
    currencies: Record<string, CurrencyDesc>,
  ): void {
  ...
  private setCurrencyControlsLoading(loading: boolean): void {
    ...
    for (const chip of this.presetChips.querySelectorAll<HTMLButtonElement>(
      ".chip",
    )) {
  ...
  private bindEvents(): void {
    ...
    for (const tab of [this.currencyTab, this.goldTab]) {
      tab.addEventListener("keydown", (event) => {
        ...
        const nextView: AppView = this.activeView === "currency" ? "gold" : "currency";
    ...
    this.amount.addEventListener("keydown", (event) => {
      if (event.key === "Enter") runConversion();
    });
    ...
    document.addEventListener("keydown", (event) => {
      if (event.altKey && event.key.toLowerCase() === "s") {
        event.preventDefault();
        if (!this.swapButton.disabled) this.swapButton.click();
      }
    });
  }
  ...
  private async copyResult(): Promise<void> {
    ...
    try {
      ...
      this.showNonBlockingError(
        "Unable to copy automatically. Select the amount and copy it manually.",
      );
    }
  }

  private showNonBlockingError(message: string): void {
  ...
  private updateChipStates(): void {
    ...
    for (const chip of this.presetChips.querySelectorAll<HTMLElement>(".chip")) {
      const chipValue = Number(chip.dataset.value);
      chip.classList.toggle(
        "active",
        Number.isFinite(amount) && chipValue === amount,
      );
    }
  }
  ...
  private showResult(result: ConversionResult): void {
    if (this.errorTimer !== undefined) clearTimeout(this.errorTimer);
    this.setConversionLoading(false);
    this.resultAmount.textContent = formatCurrency(
      result.toAmount,
      result.toCurrency,
    );
  ...
  private setGoldLoading(loading: boolean): void {
    const disabled = loading || !this.currenciesReady;
    this.goldCurrencySelect.disabled = disabled;
    this.refreshGoldButton.disabled = disabled;
  ...
  private showGoldError(message: string): void {
  ...
  private showGoldResult(result: GoldPriceResult): void {
    this.setGoldLoading(false);
  ...
  private updateGoldCalculator(): void {
    ...
    const karat: GoldKarat = this.goldKaratSelect.value === "24" ? 24 : 21;
  ...
  private async fetchRates(base: string): Promise<ExchangeRateResponse> {
    ...
    try {
      ...
      const rates = Object.fromEntries(
        Object.entries(payload.rates as Record<string, unknown>)
          .filter(([, value]) => typeof value === "number" && Number.isFinite(value))
          .map(([code, value]) => [code.toUpperCase(), value as number]),
      );
      ...
      const data: ExchangeRateResponse = { rates, timestamp };
  ...
  private async fetchGoldSpot(forceRefresh = false): Promise<GoldSpotResponse> {
    const now = Math.floor(Date.now() / 1_000);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Gold API error: ${response.status} ${response.statusText}`.trim(),
        );
      }
  ...
  private async convert(): Promise<void> {
    ...
    if (!this.currenciesReady || !fromCurrency || !toCurrency) return;
  ...
  private async loadGoldPrice(forceRefresh = false): Promise<void> {
    const requestId = ++this.goldRequestId;
    const currency = this.goldCurrencySelect.value.toUpperCase();

    if (!this.currenciesReady || !currency) return;
    ...
    try {
      ...
      const [goldSpot, rateData] = await Promise.all([
        ...
        currency === "USD"
          ? Promise.resolve<ExchangeRateResponse>({
              rates: { USD: 1 },
              timestamp: Math.floor(Date.now() / 1_000),
            })
          : this.fetchRates("USD"),
      ...
      if (requestId !== this.goldRequestId) return;
      ...
      const prices = calculateGoldPrices(
        goldSpot.priceUsdPerOunce * exchangeRate,
      );
