...
function parseOpenExchangeCurrencies(data: unknown): Record<string, CurrencyDesc> {
...
function parseFxRatesCurrencies(data: unknown): Record<string, CurrencyDesc> {
  if (!data || typeof data !== "object" || Array.isArray(data)) {
  ...
  const source =
    root.symbols && typeof root.symbols === "object" && !Array.isArray(root.symbols)
      ? (root.symbols as Record<string, unknown>)
      : root;

  return Object.fromEntries(
    Object.entries(source).map(([rawCode, value]) => {
      ...
      if (typeof value === "string") {
        ...
        description = String((value as Record<string, unknown>).name ?? code);
      }
...
export async function fetchAllCurrencies(
  timeoutMs = 8_000,
): Promise<Record<string, CurrencyDesc>> {
  const providers = [
    {
...
export function parseGoldSpotResponse(
  data: unknown,
  fallbackTimestamp = Math.floor(Date.now() / 1_000),
): GoldSpotResponse {
  if (!data || typeof data !== "object" || Array.isArray(data)) {
...
export function validateGoldWeight(
  value: string,
): { isValid: true; grams: number } | { isValid: false; error: string } {
  const trimmed = value.trim();
  if (trimmed === "") return { isValid: true, grams: 0 };
